    ...
    <>
      <div className="fixed bottom-6 right-4 md:bottom-8 md:right-8 z-[999999] group flex flex-col items-center justify-end">
        ...
        <button onClick={() => setIsOpen(!isOpen)} className="hover:scale-110 transition-all outline-none relative z-10 flex justify-center items-center w-20 h-20 md:w-28 md:h-28">
          {isOpen ? (
            <div className="w-12 h-12 md:w-14 md:h-14 bg-[#c20000] rounded-full flex justify-center items-center shadow-lg border-2 border-[#eab953]">
      ...
      {isOpen && (
        <div className="fixed bottom-28 right-4 md:bottom-40 md:right-8 z-[999999] w-[85vw] max-w-[320px] md:max-w-[360px] h-[450px] md:h-[520px] max-h-[75vh] bg-white rounded-3xl shadow-2xl flex flex-col overflow-hidden border border-[#eab953]/30 animate-fade-in origin-bottom-right">
          
          <div className="bg-gradient-to-r from-[#4a0000] via-[#810000] to-[#c20000] p-3 md:p-4 flex items-center shadow-md flex-shrink-0">
            {language && (
              <button onClick={handleBackSequence} className="mr-2 text-white hover:text-[#eab953] w-6 h-6 md:w-8 md:h-8 flex justify-center items-center transition-all">
                <i className="fa-solid fa-chevron-left text-base md:text-lg"></i>
              </button>
            )}
            
            <div className="w-10 h-10 md:w-12 md:h-12 bg-white rounded-full p-1 border-2 border-[#eab953] overflow-hidden flex justify-center items-center flex-shrink-0">
            ...
            <div className="text-left ml-2 md:ml-3 flex-grow">
               ...
               <p className="text-[#eab953] text-[8px] md:text-[10px] font-bold flex items-center gap-1">
          ...
          <div className="flex-grow bg-[#f8f9fa] p-3 md:p-4 overflow-y-auto flex flex-col gap-3 md:gap-4 scroll-smooth">
            {!language ? (
              <div className="animate-fade-in flex flex-col gap-3">
                ...
                <button onClick={() => handleLanguageSelect('ur')} className="bg-[#eab953] text-white py-2.5 px-4 rounded-xl flex justify-between items-center font-bold text-[11px] md:text-xs shadow-md">اردو (Roman Urdu) <i className="fa-solid fa-arrow-right"></i></button>
              </div>
            ) : !selectedTopic ? (
              <div className="grid grid-cols-2 gap-2 animate-fade-in">
                ...
                {currentTopics.map((topic, idx) => (
                  <button key={idx} onClick={() => handleTopicSelect(topic)} className="bg-white border border-[#eab953] text-[#810000] py-2 md:py-2.5 px-1 rounded-lg text-[10px] md:text-[11px] font-bold shadow-sm hover:bg-[#eab953] hover:text-white transition-all">{topic}</button>
                ))}
              </div>
            ) : (
              <div className="flex flex-col gap-2.5 md:gap-3 animate-fade-in">
                <div className="bg-[#eab953] text-white p-1.5 px-3 rounded-2xl rounded-tr-none shadow-sm self-end text-[9px] md:text-[11px] font-medium opacity-80">{selectedTopic}</div>
                {messages.map((msg, index) => (
                  <div key={index} className={`max-w-[85%] p-2.5 md:p-3 text-[11px] md:text-xs font-medium whitespace-pre-wrap break-words ${msg.role === 'user' ? 'bg-[#eab953] text-white rounded-2xl rounded-tr-none self-end shadow-sm' : 'bg-white border border-gray-100 text-gray-700 rounded-2xl rounded-tl-none self-start shadow-sm'}`}>{msg.text}</div>
                ))}
                {isTyping && (
          ...
          {language && selectedTopic && (
            <div className="w-full p-2.5 md:p-3 bg-white border-t z-20 shadow-inner flex-shrink-0">
              <form onSubmit={handleSendMessage} className="flex gap-2">
                <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder={language === 'en' ? "Type message..." : "Sawal likhein..."} className="flex-grow bg-gray-100 rounded-full px-3 md:px-4 py-2 md:py-2.5 text-[11px] md:text-xs outline-none focus:bg-white focus:border-[#eab953] border border-transparent transition-all shadow-inner" disabled={isTyping} />
                <button type="submit" disabled={isTyping} className={`bg-[#810000] text-white w-8 h-8 md:w-10 md:h-10 rounded-full flex justify-center items-center shadow-md transition-all ${isTyping ? 'opacity-50 cursor-not-allowed' : 'hover:bg-[#eab953]'}`}><i className="fa-solid fa-paper-plane text-xs md:text-sm"></i></button>
              </form>
            </div>
          )}
        </div>
