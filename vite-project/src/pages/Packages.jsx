...
function Packages({ currency, exchangeRates }) {
  ...
  const selectedPackage = packageData.find(p => p.id.toString() === selectedPkgId);
                    ...
                    {activeTab === 'packages' && (
                        <div className="flex flex-col md:flex-row gap-4 items-end animate-fade-in">
                            ...
                            <div className="w-full md:w-auto">
                                <button className="btn-gold w-full md:w-36 font-bold py-3 px-6 rounded-lg shadow-md h-[48px]">Search</button>
                            </div>
                        </div>
                    )}
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch animate-fade-in">
                        {packageData.slice(0, 3).map((pkg, index) => {
                            const convertedPrice = pkg.basePrice * exchangeRates[currency];
                            const totalPrice = pilgrims * convertedPrice;
                            return (
                                <div key={pkg.id} className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100 hover:shadow-lg transition flex flex-col h-full">
                                    ...
                                    <div className="p-5 flex-grow flex flex-col">
                                        ...
                                        <div className="bg-[#f8f9fa] p-4 rounded-lg border border-gray-100 mt-auto">
                                            <div className="flex justify-between text-sm mb-2"><span className="text-gray-500">Makkah Distance:</span><span className="font-bold text-gray-900">{pkg.distances.makkah}</span></div>
                                            <div className="flex justify-between text-sm"><span className="text-gray-500">Madinah Distance:</span><span className="font-bold text-gray-900">{pkg.distances.madinah}</span></div>
                                        </div>
                                        ...
                                        <div className="flex justify-between items-end mt-auto">
                                            ...
                                            <button onClick={() => setSearchParams({ pkg: pkg.id })} className="btn-gold px-5 py-2.5 rounded-md font-semibold">Book Now</button>
                                        </div>
                                    </div>
                                </div>
                            );
